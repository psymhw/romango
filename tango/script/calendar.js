...
function DefineEvent(EventDate, EventDescription, EventLink, Image, Width, Height, color) {
	var tmp;

	// Build the HTML string for this event: image (optional), link (optional), and description
	tmp = "";
	tmp = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><tr><td bgcolor='"+color+"'>"+ '<img src="' + Image + '"  width="' + Width + '" height="' + Height + '" align="left" valign="top"></td>'
	tmp = tmp + "<td bgcolor='"+color+"'><a href=\"" + EventLink + "\">";
	tmp = tmp + EventDescription+"</td></tr></table>";
	/*
	if (Image != "")
...
function getMo(str) {
	return parseInt(str.substring(4));
}
...
function Calendar( ) {
	...
	//out.println("calendar.js - mo: "+mo);
	//System.out.println("calendar.js - yr: "+yr);
	...
	document.write("<TABLE BORDER=2 width='100%' BGCOLOR="+ColorBackground+"><TR><TD ALIGN=CENTER COLSPAN=7><FONT SIZE="+FontSize+"><B>"+months[mo]+" "+yr+"</B></FONT></TD></TR><TR>");
	for (var i=1;i<=7;i++){
