...
public class iTunesParser extends DefaultHandler
{
	...
	private TrackData track = new TrackData();
	...
	**/
	...
	public void endElement(String uri, String localName, String qName)
	{
		...
		else if(qName.equals("dict") && isTracks)
		{
			...
			{
				//end of all tracks
				data.tracks = new TrackData[tracksVector.size()];
				for(int i=0; i<tracksVector.size(); i++)
					data.tracks[i] = (TrackData)tracksVector.elementAt(i);
				isTracks = false;
			}
			else
			{
    ...
    {
      track.rating = Integer.parseInt(buffer.toString());
