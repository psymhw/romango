...
public class MidiSequence {
    ...
    public boolean load(String midifile) 
    {
      URL url = null;
      try {
    	    url=this.getClass().getResource("/resources/sounds/"+midifile);
         	//url = new URL(codeBase+"resources/sounds/"+midifile);
           //System.out.println("MidiSequence - "+url.toURI());
         } catch (Exception e) { e.printStackTrace();}
     try {
           //load the midi file into the sequencer
           filename = midifile;
           song = MidiSystem.getSequence(url);
