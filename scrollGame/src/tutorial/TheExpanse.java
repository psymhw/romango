...
public class TheExpanse extends GameWorld {
    ...
    *
	...
	EventHandler fireOrMove = new EventHandler() {
            ...
            MouseEvent event = (MouseEvent)me;
        ...
        EventHandler changeWeapons = new EventHandler() {
            @Override
            public void handle(Event ke) {
            	
            KeyEvent event=(KeyEvent)ke;
...
private void removeMissiles(Missile missile) {         
	...
	if (missile.node.getTranslateX() > (getGameSurface().getWidth() -
                ...
                missile.node.getTranslateX() < 0) {             getSpriteManager().addSpritesToBeRemoved(missile);            
